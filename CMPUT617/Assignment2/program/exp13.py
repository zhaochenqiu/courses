...
for itr in range(1000):
    ...
    tempdata = data.detach().cpu().numpy()
    print(tempdata)
    print(type(tempdata))
    print(data.shape)

    print("before sample_batch")
    batch = sample_batch(tempdata,batch_size=batch_size), sample_batch(tempdata,batch_size=batch_size,sample_mode='marginal')
    print("after sample_batch")

    print("before learn_mine")
    mi_lb, ma_et = learn_mine(batch, mine_net, mine_net_optim, ma_et)
    print("after learn_mine")
