...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(9, 32, 10, 1)
...
#    path_im = "D:\dataset\microscopy"
    ...
    pickle.dump(train_fs, open('train_fs_test.txt','wb'))
    pickle.dump(valid_fs, open('valid_fs_test.txt','wb'))
    pickle.dump(test_fs, open('test_fs_test.txt','wb'))


    train_fs = pickle.load(open('train_fs_test.txt','rb'))
    valid_fs = pickle.load(open('valid_fs_test.txt','rb'))
    test_fs = pickle.load(open('test_fs_test.txt','rb'))
...
#             batch_size=args.batch_size, shuffle=True, **kwargs)
    ...
    torch.save(model.state_dict(),"mic_network_test.pt")



    smodel = model = Net().to(device)
    smodel.load_state_dict(torch.load("mic_network_test.pt"))
