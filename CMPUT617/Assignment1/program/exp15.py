...
def getMicImdbbyFile(fullfile, num = 1000, radius = 28):
...
def main():
    ...
    num = 10000
    patches_pos, labs_pos = getMicImdbbyFile(fullfs_pos[0], num)
    patches_neg, labs_neg = getMicImdbbyFile(fullfs_neg[0], num)

    traindata = np.append(patches_pos, patches_neg, axis = 0)
    trainlabs = np.append(labs_pos, labs_neg)
    traindata = MicDataset(traindata, trainlabs)


    num = 10000
    patches_pos, labs_pos = getMicImdbbyFile(fullfs_pos[1], num)
    patches_neg, labs_neg = getMicImdbbyFile(fullfs_neg[1], num)

    testdata = np.append(patches_pos, patches_neg, axis = 0)
    testlabs = np.append(labs_pos, labs_neg)
    testdata = MicDataset(testdata, testlabs)
