...
I = io.imread('../image_fixed.jpg').astype(np.float32)
J = io.imread('../image_moving.jpg').astype(np.float32)

I = skimage.color.rgb2gray(I)
J = skimage.color.rgb2gray(J)
...
for level in torch.arange(4, -1, -1):
    ...
    tval = 0.5*threshold_otsu(I.cpu().numpy()) # reduce Otsu threshold value a bit to cover slightly wider areas
